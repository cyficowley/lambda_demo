...
```bash
test_app$ sam build --use-container
```
...
```bash
test_app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
test_app$ sam local start-api
test_app$ curl http://localhost:3000/
```
...
```bash
test_app$ sam logs -n HelloWorldFunction --stack-name test_app --tail
```
...
```bash
test_app$ pip install pytest pytest-mock --user
test_app$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name test_app
```
